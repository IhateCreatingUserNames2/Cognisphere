...
try:
    ...
    traceback.print_exc()
try:
    print("Initializing AgentRegistryService...")
    # Option 1: Initialize directly if services_container does it
    # (services_container.initialize_services should now handle this)

    # Option 2: If you prefer separate initialization in app.py
    # agent_registry_service_instance = AgentRegistryService()
    # services_container.initialize_agent_registry_service(agent_registry_service_instance)

    # Ensure it's initialized (it should be by services_container.initialize_services)
    if services_container.get_agent_registry_service() is None:
        print("CRITICAL: AgentRegistryService failed to initialize in services_container.")
        # Attempt a direct initialization here as a last resort
        agent_registry_service_instance = AgentRegistryService()
        services_container.initialize_agent_registry_service(agent_registry_service_instance)

    print("AgentRegistryService initialization check complete.")

    # Example: Register a few default agents if the registry is empty
    # This is for testing; in a real system, these would be managed via an API or UI
    registry = services_container.get_agent_registry_service()
    if registry and not registry.list_agents():
        print("Agent registry is empty. Registering default agents...")
        from data_models.registered_agent import RegisteredAgent

        # Memory Agent
        memory_agent_config = RegisteredAgent(
            name="MemoryAgent",
            description="Specialized agent for memory storage and retrieval operations.",
            capabilities=["memory recall", "store information", "remember facts"],
            module_path="agents.memory_agent",
            creation_function="create_memory_agent",
            default_model=config.MODEL_CONFIG.get("memory", "openai/gpt-4o-mini"),
            initial_instruction_prompt="You are the Memory Agent. Your role is to manage the storage and retrieval of memories. Respond factually without conversational fluff."
        )
        registry.register_agent(memory_agent_config)
        print(f"Registered: {memory_agent_config.name}")

        # Narrative Agent
        narrative_agent_config = RegisteredAgent(
            name="NarrativeAgent",
            description="Specialized agent for narrative thread creation and management.",
            capabilities=["story creation", "narrative management", "summarize experiences", "track storylines"],
            module_path="agents.narrative_agent",
            creation_function="create_narrative_agent",
            default_model=config.MODEL_CONFIG.get("narrative", "openai/gpt-4o-mini"),
            initial_instruction_prompt="You are the Narrative Agent. Your role is to create and manage narrative threads. Respond factually."
        )
        registry.register_agent(narrative_agent_config)
        print(f"Registered: {narrative_agent_config.name}")

        # Identity Agent
        identity_agent_config = RegisteredAgent(
            name="IdentityAgent",
            description="Specialized agent for identity creation, management, and switching.",
            capabilities=["identity creation", "identity management", "switch persona", "update profile"],
            module_path="agents.identity_agent",
            creation_function="create_identity_agent",
            default_model=config.MODEL_CONFIG.get("orchestrator", "openai/gpt-4o-mini"),
            # Often uses orchestrator model
            initial_instruction_prompt="You are the Identity Agent. Your role is to manage user and system identities. Respond factually."
        )
        registry.register_agent(identity_agent_config)
        print(f"Registered: {identity_agent_config.name}")

        # MCP Agent
        mcp_agent_config = RegisteredAgent(
            name="MCPAgent",
            description="Agent for interacting with external tools and services via the Model Context Protocol (MCP).",
            capabilities=["mcp", "external tools", "api integration", "list mcp tools", "call mcp tool"],
            module_path="agents.mcp_agent",
            creation_function="create_mcp_agent",
            default_model=config.MODEL_CONFIG.get("orchestrator", "openai/gpt-4o-mini"),
            initial_instruction_prompt="You are the MCP Agent. You list and call tools from external MCP servers. Respond with raw JSON output from tools."
        )
        registry.register_agent(mcp_agent_config)
        print(f"Registered: {mcp_agent_config.name}")


except Exception as e:
    print(f"Error initializing AgentRegistryService or default agents: {e}")
    traceback.print_exc()
...
# ---- Initialize MCP Manager and Connect to Servers (CORRECTED BLOCK) ----
try:
    from mcpIntegration.mcp_manager import MCPManager
    from mcpIntegration.server_installer import MCPServerManager as MCPRegistryManager
    from google.adk.tools import FunctionTool, ToolContext  # For MCP tool handlers later

    print("Initializing MCP Manager (NEW BLOCK)...")
    mcp_manager_instance = MCPManager()  # Instance of your LATEST MCPManager
    app_globals.mcp_manager = mcp_manager_instance # Store globally

    mcp_registry = MCPRegistryManager()  # Your server config storage/registry

    # 1. Get server configurations as a LIST OF DICTIONARIES
    all_server_configs_list: List[Dict[str, Any]] = mcp_registry.list_servers()
    #    (Ensure mcp_registry.list_servers() returns List[Dict],
    #     e.g., by doing `list(self.servers.values())` in MCPServerManager
    #     if self.servers is a Dict[str, Dict])

    print(f"DEBUG: Number of server configs from registry: {len(all_server_configs_list)}")
    if all_server_configs_list and isinstance(all_server_configs_list, list): # Extra check
        # Ensure the first item (if list is not empty) is a dictionary
        if all_server_configs_list and not isinstance(all_server_configs_list[0], dict):
            print(f"ERROR DEBUG: First item in server_configs list is NOT a dict: {type(all_server_configs_list[0])}")
        elif all_server_configs_list: # It's a list and first item is a dict (or list is empty)
             print(f"DEBUG: First server config item type: {type(all_server_configs_list[0])}, content: {all_server_configs_list[0]}")
    elif not all_server_configs_list:
        print("DEBUG: all_server_configs_list is empty.")
    else: # Not a list
        print(f"ERROR DEBUG: all_server_configs_list is NOT a list: {type(all_server_configs_list)}")


    if all_server_configs_list and isinstance(all_server_configs_list, list): # Check it's a list
        # 2. Call discover_and_connect_servers with this LIST
        mcp_adk_tools = run_async_in_event_loop(
            mcp_manager_instance.discover_and_connect_servers(all_server_configs_list)  # PASS THE LIST
        )

        if isinstance(mcp_adk_tools, list):
            # Assuming app_globals.mcp_tools is intended to store ADK-wrapped MCP tools
            # that can be used by agents.
            if not hasattr(app_globals, 'mcp_tools') or app_globals.mcp_tools is None:
                app_globals.mcp_tools = [] # Initialize if it doesn't exist
            app_globals.mcp_tools.extend(mcp_adk_tools)
            print(f"✅ MCP servers processed. Discovered {len(mcp_adk_tools)} ADK-wrapped MCP tools.")
        else:
            print(f"⚠️ discover_and_connect_servers did not return a list of tools. Type: {type(mcp_adk_tools)}")
            if mcp_adk_tools is not None:
                 print(f"   Content: {mcp_adk_tools}")
    else:
        print("No MCP server configurations found in registry (or config list is not a list) to connect to at startup.")

    # --- MCP Tool Handlers (for Orchestrator/MCPAgent to use as ADK FunctionTools) ---
    # This part defines how your *Cognisphere agents* will call out to MCP services.
    # It's separate from discovering tools *from* external MCP servers.
    from mcpIntegration.tool_handlers import list_mcp_tools, call_mcp_tool

    # These are FunctionTools that your *MCPAgent* will use.
    # They rely on app_globals.mcp_manager being set up correctly.
    cognisphere_mcp_function_tools = [
        FunctionTool(list_mcp_tools),
        FunctionTool(call_mcp_tool)
    ]
    app_globals.function_tools = cognisphere_mcp_function_tools # Make them globally available for MCPAgent
    print(f"Initialized {len(cognisphere_mcp_function_tools)} MCP function tools for Cognisphere agents.")


except ImportError as e:
    print(f"MCP components not available - proceeding without MCP support: {e}")
except Exception as e:
    print(f"❌ Error during MCP initialization/auto-connection: {e}")
    traceback.print_exc()
# ---- END OF MCP INITIALIZATION BLOCK ----
...
try:
    ...
    def timestamp_to_datetime(timestamp):
        """Convert Unix timestamp to ISO formatted datetime string."""
        from datetime import datetime
    ...
    def _append_safe(session, event):
        if getattr(event, "content", None) is not None:
            try:
                ...
                if "last_update_time" in str(e):
                    print(f"Timestamp error, forcing reset of timestamp")

                    # Connect directly to database to update the timestamp
                    import sqlite3
                    conn = sqlite3.connect("cognisphere_sessions.db")
                    cursor = conn.cursor()

                    # Convert timestamp to proper format and update directly
                    timestamp_iso = timestamp_to_datetime(session.last_update_time)
                    ...
                    ''', (timestamp_iso, session.app_name, session.user_id, session.id))

                    conn.commit()
                    conn.close()

                    # Try the append operation again
                    return _orig_append(session, event)
                else:
                    raise
        else:
...
@app.route('/api/util/reset_session_timestamps', methods=['POST'])
def reset_session_timestamps():
    """Reset timestamps for all sessions to fix inconsistencies."""
    try:
        # Get all sessions
        import sqlite3
        conn = sqlite3.connect("cognisphere_sessions.db")
        ...
        # Update all timestamps to current time in ISO format
        cursor.execute('''
            UPDATE sessions
            SET update_time = ?
        ...
        return jsonify({
            "status": "success",
            "message": "All session timestamps reset successfully"
        })
    except Exception as e:
        return jsonify({
            "status": "error",
            "message": f"Error resetting timestamps: {str(e)}"
        }), 500
